...
static PyObject* build_object(PyObject* self, PyObject* args) {
  ...
  Py_ssize_t size;
  const char* name;
  if (!PyArg_ParseTuple(args, "O&sns", &GetClientContext,
...
static PyObject* get_mutable_buffer(PyObject* self, PyObject* args) {
  ...
  return PyMemoryView_FromMemory(reinterpret_cast<char*>(buffer->mutable_data()), buffer->size(), PyBUF_WRITE);
}
...
static PyObject* get_object(PyObject* self, PyObject* args) {
  ClientContext* context;
  const char* object_id;
  if (!PyArg_ParseTuple(args, "O&s", &GetClientContext, &context, &object_id)) {
    return NULL;
  }
  ...
  return PyMemoryView_FromMemory(const_cast<char*>(data), buffer.size(), PyBUF_READ);
}
...
static PyObject* list_objects(PyObject* self, PyObject* args) {
  ...
  PyObject* construct_durations = PyList_New(infos.size());
  ...
  for (size_t i = 0; i < infos.size(); ++i) {
    ...
    PyList_SetItem(construct_durations, i, PyInt_FromLong(infos[i].construct_duration));
  ...
  PyTuple_SetItem(result, 3, construct_durations);
